...
class Encoder(Module):
    def __init__(self, hidden_dim, emb_dim, num_layers,
        ...
        self.ohe_lin = Linear(1, hidden_dim)
    ...
    def forward(self, z:Tensor, pos: Tensor,
        ...
        # s = torch.ones((pos.size(0),4), device=pos.device)
        # s = self.ohe_lin(s)
        s = self.ohe_lin(z)
...
class Decoder(Module):
    def __init__(self, hidden_dim, emb_dim, num_layers,
        ...
        self.start_pos = nn.Parameter(torch.randn(29, 3))
    ...
    def forward(self, s: Tensor, rot: Tensor, transl: Tensor, perm: Tensor,
        ...
        s = s[mask].view(-1, self.hidden_dim)
        ...
        pos = pos_start.reshape(-1, 3)[:s.shape[0]]
...
class Model(Module):
    ...
    def forward(self, data: Union[Data, Batch], tau: float = 1.0, hard: bool = False, do_rot: bool = True, **kwargs):
        ...
        shape_emb, point_emb, rot, transl, vout = self.encoder(z=data.z.view(-1,1).to(torch.float), pos=data.pos,
                                                               batch=batch,
